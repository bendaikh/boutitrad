@props(['label', 'menuKey', 'icon' => 'box', 'active' => false, 'open' => false])

<div>
    <button
        type="button"
        @click="openMenu = openMenu === '{{ $menuKey }}' ? '' : '{{ $menuKey }}'; sessionStorage.setItem('adminOpenMenu', openMenu)"
        class="admin-nav-group-btn {{ $active ? 'admin-nav-group-btn--active' : '' }}"
    >
        <span class="flex items-center gap-3">
            @switch($icon)
                ...
                @case('users')
                    ...
                    @break
                @case('truck')
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10m10 0h4m-4 0a2 2 0 100 4 2 2 0 000-4zm-8 0a2 2 0 100 4 2 2 0 000-4z"/></svg>
                    @break
                @case('cog')
                    ...
                    @break
            @endswitch
            <span class="font-tradition tracking-wide">{{ $label }}</span>
        </span>
        <svg class="w-4 h-4 shrink-0 text-gold-300/70 transition-transform duration-150" :class="openMenu === '{{ $menuKey }}' && 'rotate-90'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
    </button>

    <div
        x-show="openMenu === '{{ $menuKey }}'"
