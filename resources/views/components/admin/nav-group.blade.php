...
<div>
    <button
        ...
        class="w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ $active ? 'bg-brand-50 text-brand-700 dark:bg-brand-900/40 dark:text-brand-300' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-800 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-100' }}"
    >
        ...
        <svg class="w-4 h-4 shrink-0 transition-transform duration-75" :class="openMenu === '{{ $menuKey }}' && 'rotate-90'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div
        ...
        class="mt-1 ml-3 pl-3 border-l border-slate-200 dark:border-slate-700 space-y-1"
    >
        {{ $slot }}
    </div>
</div>
