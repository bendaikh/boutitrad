<x-admin-layout title="Commande {{ $order->reference }}">
    <div class="flex flex-wrap gap-3 mb-6">
        <a href="{{ route('orders.invoice', $order) }}" target="_blank" class="px-4 py-2 bg-slate-800 text-white rounded-lg text-sm hover:bg-slate-700">Facture / Imprimer</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-semibold">{{ $order->reference }}</h2>
                ...
                <table class="w-full text-sm">
                    ...
                    <tfoot class="border-t">
                        <tr><td colspan="3" class="px-4 py-2 text-right text-slate-500">Sous-total</td><td class="px-4 py-2 text-right">{{ number_format($order->subtotal, 2, ',', ' ') }} DH</td></tr>
                        @if($order->discount > 0)<tr><td colspan="3" class="px-4 py-2 text-right text-slate-500">Remise</td><td class="px-4 py-2 text-right text-red-600">-{{ number_format($order->discount, 2, ',', ' ') }} DH</td></tr>@endif
                        <tr><td colspan="3" class="px-4 py-2 text-right font-semibold">Total</td><td class="px-4 py-2 text-right font-bold text-indigo-600">{{ number_format($order->total, 2, ',', ' ') }} DH</td></tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                ...
                <div class="space-y-2">
                    @foreach($order->statusHistories as $history)
                        <div class="flex items-center gap-3 text-sm border-l-2 border-indigo-200 pl-3">
                            ...
                            @if($history->notes)<span class="text-slate-500">- {{ $history->notes }}</span>@endif
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm text-sm space-y-2">
                ...
                <div><span class="text-slate-500">Commercial:</span> {{ $order->commercial?->name ?? '-' }}</div>
                <div><span class="text-slate-500">Livreur:</span> {{ $order->livreur?->name ?? '-' }}</div>
                <div><span class="text-slate-500">Créée le:</span> {{ $order->created_at->format('d/m/Y H:i') }}</div>
            </div>
            ...
            <form method="POST" action="{{ route('orders.status', $order) }}" class="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-3">
                ...
                <button type="submit" class="w-full py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700">Mettre à jour</button>
            </form>
            @endif
        </div>
    </div>
