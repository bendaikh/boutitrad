<x-admin-layout title="{{ isset($product) ? 'Modifier produit' : 'Nouveau produit' }}">
    <form method="POST" action="{{ isset($product) ? route('products.update', $product) : route('products.store') }}" class="max-w-2xl bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
        ...
        <label class="flex items-center gap-2"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $product->is_active ?? true)) class="rounded text-indigo-600"> Actif</label>
        <div class="flex gap-3"><button type="submit" class="px-5 py-2 bg-indigo-600 text-white rounded-lg text-sm">Enregistrer</button><a href="{{ route('products.index') }}" class="px-5 py-2 bg-slate-100 rounded-lg text-sm">Annuler</a></div>
    </form>
</x-admin-layout>
