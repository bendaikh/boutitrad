    ...
    <form method="POST" action="{{ route('settings.update') }}" class="max-w-2xl bg-white rounded-xl border p-6 shadow-sm space-y-6">
        ...
        <div><h3 class="font-semibold mb-3">Paramètres commerciaux</h3>
            <div class="grid grid-cols-2 gap-3">
                <div><label class="text-sm text-slate-500">Préfixe commandes</label><input type="text" name="order_prefix" value="{{ $settings['order_prefix'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
                <div><label class="text-sm text-slate-500">Taux commission (%)</label><input type="number" step="0.1" name="commission_rate" value="{{ $settings['commission_rate'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
                <div><label class="text-sm text-slate-500">Frais livraison</label><input type="number" step="0.01" name="delivery_fee" value="{{ $settings['delivery_fee'] }}" class="w-full rounded-lg border-slate-300 text-sm"></div>
            </div>
        </div>
