<x-admin-layout title="{{ isset($user) ? 'Modifier utilisateur' : 'Nouvel utilisateur' }}">
    <form method="POST" action="{{ isset($user) ? route('users.update', $user) : route('users.store') }}" class="max-w-lg bg-white rounded-xl border p-6 shadow-sm space-y-4">
        ...
        <label class="flex items-center gap-2"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', isset($user) ? $user->is_active : true)) class="rounded text-brand-600"> Actif</label>
        <div class="flex gap-3"><button type="submit" class="px-5 py-2 bg-brand-600 text-white rounded-lg text-sm">Enregistrer</button><a href="{{ route('users.index') }}" class="px-5 py-2 bg-slate-100 rounded-lg text-sm">Annuler</a></div>
    </form>
</x-admin-layout>
